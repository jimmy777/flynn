...
func (j *Job) lock() {
	j.setLockedUntil(time.Now().Add(10 * time.Second))
	go func() {
		for {
			select {
			case <-j.release:
				j.setLockedUntil(time.Now())
				close(j.unlocked)
				return
...
func (j *Job) setLockedUntil(t time.Time) {
	if _, err := j.pool.Exec("que_set_lock", t, j.Queue, j.Priority, j.RunAt, j.ID); err != nil {
		// TODO: handle error
	}
}
